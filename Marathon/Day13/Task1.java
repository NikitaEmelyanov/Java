 ...
 * MessageDatabase - класс, который будет заниматься хранением сообщений Test - класс, где в методе
 * main() мы будем тестировать нашу соц. сеть
 ...
 *
 * - public boolean isFriend(User user) - возвращает True, если пользователь user является другом и
 * False, если пользователь user не является другом. Подумайте, что такое дружба в контексте соц. сетей.
 ...
 * сообщения. Также, конструктор должен назначать полю date текущее время (то есть время создания
 * объекта Message).
 * Методы:
 * - Геттеры на все поля
 ...
 * TO: 'Имя получателя'
 * ON: Sun Aug 30 19:07:34 MSK 2020 'Текст сообщения'
 * *в поле ON должна быть дата создания объекта класса Message Класс Message Database должен иметь
 * следующую структуру:
 * Поля:
 * - Статическое поле “сообщения” (англ. messages), которое будет хранить список
 * из сообщений (объектов класса Message). Это поле должно инициализироваться пустым списком. Этот
 * список и есть наша условная “база данных”, которая хранит все сообщения в соц. сети.
 *
 * Конструктор:
 *Нет конструктора. Объекты класса Message Database создаваться не будут(все методы и поля статические).
 * Методы:
 ...
 * user1: Привет!
 * user2: Привет, user1! user1: Как у тебя дела? user2: Все ок, спасибо :)
...
public class Task1 {
    public static void main(String[] args) {
        ...
        user2.sendMessage(user1, "i'm user 2.Nice to meet you.");
